...
public partial class EWindowViewModelBase : ViewModelBase<Window>
{
    ...
    protected virtual void WindowFadeOut(Action? action, double factor)
    {
        ...
        Methods.SimpleDoubleAnimation(FadeDuration.Factor(factor), 0, null, () => {
            SystemCommands.MinimizeWindow(ownerWindow);
            ownerWindow.SetValue(UIElement.OpacityProperty, Option.GetInstance().Option_Opacity);
            action?.Invoke();
    ...
    protected virtual void WindowFadeIn(Action? action, double factor)
    {
        ...
        Methods.SimpleDoubleAnimation(FadeDuration.Factor(0), 0, null, null, ownerWindow, UIElement.OpacityProperty);
        Methods.SimpleDoubleAnimation(FadeDuration.Factor(factor), Option.GetInstance().Option_Opacity, null, () => {
            ownerWindow.SetValue(UIElement.OpacityProperty, Option.GetInstance().Option_Opacity);
            action?.Invoke(); 
        }, ownerWindow, UIElement.OpacityProperty);
    }
