    ...
    public class VMCommand : ViewModelBase, ICommand
    {
        ...
        public VMCommand(Func<object?, Task> executeAsyncAction, Func<object?, bool>? canExecuteFunc, string translateName, Func<string, string>? translatedTextBuilder)
        {
            ...
            ExecuteAction = o =>
            {
                ...
                task.ContinueWith(t =>
                {
                    if (t.Exception is not null) WpfLog.UILogger.Error(t.Exception); ;
        ...
        public void InvokeSync(object? parameter, Dispatcher? dispatcher = null)
        {
            ...
            dispatcher.Invoke(() =>
            {
                WpfLog.UILogger.Log("Execute action on ui dispatcher");
                Execute(parameter);
                runningTask = RunningTask;
            });
            if (runningTask is not null)
            {
                WpfLog.UILogger.Log("Async action will blocks the thread...");
                try
                {
                    runningTask.Wait();
                }
                ...
                {
                    WpfLog.UILogger.Log("Async action completed.");
                }
            }
            else WpfLog.UILogger.Log("Sync action wont blocks the thread.");
        }
