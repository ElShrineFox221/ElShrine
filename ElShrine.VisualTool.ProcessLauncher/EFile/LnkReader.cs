...
namespace ElShrine.VisualTool.ProcessLauncher.EFile
{
        ...
        public static Icon GetFileIcon(string filePath, int iconIndex = 0)
        {
            ...
            static Icon? extractIconFromFile(string filePath, int iconIndex)
            {
                ...
                if (!string.IsNullOrWhiteSpace(filePath) && File.Exists(filePath))
                {
                    ...
                    if (count > 0 && largeIcons[0] != nint.Zero) result = Icon.FromHandle(largeIcons[0]);
                }
                try
                {
                    ...
                    if (!finished && !string.IsNullOrEmpty(info.TargetPath))
                    {
                        ...
                        if (expandedPath.StartsWith("::", StringComparison.Ordinal))
                        {
                            try
                            {
                                ...
                                nint hImg = SHGetFileInfo(expandedPath, 0, ref shinfo, (uint)Marshal.SizeOf(shinfo), SHGFI_ICON | SHGFI_LARGEICON);
                                if (shinfo.hIcon != nint.Zero) result = Icon.FromHandle(shinfo.hIcon);
                            }
                            catch { }
                        }
